...
async def get_lap_telemetry(dealer, logger, session_dir, app_version, *, driver_index, lap_num,
    ...
    try:
        query = LapTelemetryQuery(driver_index=driver_index, lap_num=lap_num, channels=channels,
    ...
    if session_slug is not None:
        ...
        result = await asyncio.to_thread(safe_query_reference, session_dir, reference, query, logger)
        return {**result, **context}

    try:
        reply = await dealer.request(str(PngAppId.BACKEND), "lap-telemetry-request", query.model_dump())
    except Exception:
